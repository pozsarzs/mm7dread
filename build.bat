@echo off
rem  MM7DRead v0.4 * Status reader program for MM7D device
rem  Copyright (C) 2020-2023 Pozsár Zsolt pozsarzs@gmail.com
rem  build.bat
...
echo.
%PPC% %FPFLAG1% %FPFLAG2% %FPFLAG3% %FPFLAG4% mm7dread.lpr
echo.
...
if not exist "source\lib\%ARCH%-%OS%\mm7dread.exe" ( echo Error: firstly run "build.bat" to compile source code! & goto end )
...
config\mkshortcut.vbs /target:"%INSTDIR%\%NAME%.exe" /shortcut:"%USERPROFILE%\Desktop\MM7DRead"
echo.
...
rd /s "%INSTDIR%"
del /q "%USERPROFILE%\Desktop\MM7DRead.lnk"
goto end
